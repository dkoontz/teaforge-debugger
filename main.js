...
// Set app name explicitly for development mode
app.setName("TeaForge Debugger");

// Configure the About panel to show app info instead of Electron info
const packageJson = require("./package.json");
app.setAboutPanelOptions({
  applicationName: "TeaForge Debugger",
  applicationVersion: packageJson.version,
  version: "", // Hide the build number
  copyright: "© 2025-2026",
});
...
// Handle auto-updater errors gracefully (e.g., no internet, no releases yet)
autoUpdater.on("error", (err) => {
  log.info(
    "Auto-updater error (this is normal if no releases exist yet):",
    err.message,
  );
});
...
const autoOpenFile = openFileArg
  ? openFileArg.substring("--open-file=".length)
  : null;
...
app.whenReady().then(() => {
  ...
  // Check for updates when packaged (not in development)
  if (app.isPackaged) {
