...
{-| Result of searching all fields in a LogEntry.

Contains separate match information for each section of the entry:
message, effects, before model, and after model.

-}
type alias EntrySearchResult =
    { -- All matches with their locations, for navigation
      allMatches : List MatchLocation
    , totalMatchCount : Int

    -- After model matches (for state tree highlighting)
    , afterMatches : List TreePath
    , afterPathsWithMatches : Set String

    -- Before model matches
    , beforeMatches : List TreePath
    , beforePathsWithMatches : Set String

    -- Message payload matches
    , payloadMatches : List TreePath
    , payloadPathsWithMatches : Set String

    -- Effect matches: Dict from effect index to (name matches, data paths)
    , effectNameMatches : Set Int
    , effectDataMatches : Dict Int (List TreePath)
    , effectDataPathsWithMatches : Dict Int (Set String)

    -- Simple string match for message name
    , messageNameMatches : Bool
    }


{-| Identifies where a match was found in the entry.

Used for match navigation to know which section to scroll to.

-}
type MatchLocation
    = InMessageName
    | InMessagePayload TreePath
    | InEffectName Int
    | InEffectData Int TreePath
    | InModelBefore TreePath
    | InModelAfter TreePath


{-| Empty search result for initialization.
-}
emptyEntrySearchResult : EntrySearchResult
emptyEntrySearchResult =
    { allMatches = []
    , totalMatchCount = 0
    , afterMatches = []
    , afterPathsWithMatches = Set.empty
    , beforeMatches = []
    , beforePathsWithMatches = Set.empty
    , payloadMatches = []
    , payloadPathsWithMatches = Set.empty
    , effectNameMatches = Set.empty
    , effectDataMatches = Dict.empty
    , effectDataPathsWithMatches = Dict.empty
    , messageNameMatches = False
    }
...
Searches across:
  ...
  - Message name (simple string match)
  - Message payload (JSON tree)
  - Effect names and data (for each effect)
  - Model before (JSON tree)
  - Model after (JSON tree)
...
searchEntry query entry =
    if String.isEmpty (String.trim query) then
        ...
        let
            ...
            -- Search message name
            messageNameMatches =
                matchesQuery normalizedQuery entry.message.name

            -- Search message payload
            payloadMatches =
                searchInValue normalizedQuery entry.message.payload

            payloadPathStrings =
                List.map pathToStringInternal payloadMatches |> Set.fromList

            -- Search effects
            effectResults =
                List.indexedMap (searchEffect normalizedQuery) entry.effects
            ...
            effectDataMatches =
                effectResults
                    |> List.map (\( idx, _, dataMatches ) -> ( idx, dataMatches ))
                    |> Dict.fromList

            effectDataPathsWithMatches =
                Dict.map (\_ paths -> List.map pathToStringInternal paths |> Set.fromList) effectDataMatches

            -- Search before model
            beforeMatches =
                searchInValue normalizedQuery entry.modelBefore

            beforePathStrings =
                List.map pathToStringInternal beforeMatches |> Set.fromList

            -- Search after model
            afterMatches =
                searchInValue normalizedQuery entry.modelAfter

            afterPathStrings =
                List.map pathToStringInternal afterMatches |> Set.fromList

            -- Build the combined match list for navigation
            allMatches =
                (if messageNameMatches then
                    [ InMessageName ]

                 else
                    []
                )
                    ++ List.map InMessagePayload payloadMatches
                    ++ List.concatMap
                        (\( idx, nameMatch, dataMatches ) ->
                            (if nameMatch then
                                [ InEffectName idx ]
                                ...
                                ++ List.map (InEffectData idx) dataMatches
                        )
                        effectResults
                    ++ List.map InModelBefore beforeMatches
                    ++ List.map InModelAfter afterMatches
        in
        { allMatches = allMatches
        , totalMatchCount = List.length allMatches
        , afterMatches = afterMatches
        , afterPathsWithMatches = afterPathStrings
        , beforeMatches = beforeMatches
        , beforePathsWithMatches = beforePathStrings
        , payloadMatches = payloadMatches
        , payloadPathsWithMatches = payloadPathStrings
        , effectNameMatches = effectNameMatches
        , effectDataMatches = effectDataMatches
        , effectDataPathsWithMatches = effectDataPathsWithMatches
        , messageNameMatches = messageNameMatches
        }


{-| Search a single effect and return (index, nameMatches, dataMatches).
-}
searchEffect : String -> Int -> Effect -> ( Int, Bool, List TreePath )
searchEffect query idx effect =
    let
        nameMatches =
            matchesQuery query effect.name

        dataMatches =
            searchInValue query effect.data
    in
    ( idx, nameMatches, dataMatches )
